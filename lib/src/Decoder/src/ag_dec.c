...
#define N_MAX_MEAN_CLAMP		((uint32_t)0xffffu)
#define N_MEAN_CLAMP_VAL		((uint32_t)0xffffu)
...
#else
#define ALACD_CLZ_COUNT32	((uint32_t)(32u))
#define ALACD_CLZ_COUNT16	((uint32_t)(16u))
#define ALACD_CLZ_COUNT08	((uint32_t)(8u))
#define ALACD_CLZ_COUNT04	((uint32_t)(4u))
#define ALACD_CLZ_COUNT02	((uint32_t)(2u))
#define ALACD_CLZ_COUNT01	((uint32_t)(1u))
#define ALACD_CLZ_COUNT00	((uint32_t)(0u))

#define ALACD_CLZ_MMASK16	((uint32_t)(0xFFFF0000u))
#define ALACD_CLZ_MMASK08	((uint32_t)(0xFF000000u))
#define ALACD_CLZ_MMASK04	((uint32_t)(0xF0000000u))
#define ALACD_CLZ_MMASK02	((uint32_t)(0xC0000000u))
#define ALACD_CLZ_MMASK01	((uint32_t)(0x80000000u))

static inline uint32_t lead( uint32_t m )
{
	uint32_t n = ALACD_CLZ_COUNT00;
	if ( m == (uint32_t)0u ) {
		return	ALACD_CLZ_COUNT32;
	} else {
		if ( (m & ALACD_CLZ_MMASK16) == (uint32_t)0u ) {
			n  += ALACD_CLZ_COUNT16;
			m <<= ALACD_CLZ_COUNT16;
		} else {
			/* DO NOTHING */
		} /* end if */
		if ( (m & ALACD_CLZ_MMASK08) == (uint32_t)0u ) {
			n  += ALACD_CLZ_COUNT08;
			m <<= ALACD_CLZ_COUNT08;
		} else {
			/* DO NOTHING */
		} /* end if */
		if ( (m & ALACD_CLZ_MMASK04) == (uint32_t)0u ) {
			n  += ALACD_CLZ_COUNT04;
			m <<= ALACD_CLZ_COUNT04;
		} else {
			/* DO NOTHING */
		} /* end if */
		if ( (m & ALACD_CLZ_MMASK02) == (uint32_t)0u ) {
			n  += ALACD_CLZ_COUNT02;
			m <<= ALACD_CLZ_COUNT02;
		} else {
			/* DO NOTHING */
		} /* end if */
		if ( (m & ALACD_CLZ_MMASK01) == (uint32_t)0u ) {
			n  += ALACD_CLZ_COUNT01;
		} else {
			/* DO NOTHING */
		} /* end if */
...
static inline uint32_t ALWAYS_INLINE lg3a( uint32_t x)
{
    uint32_t result;
...
getstreambits( uint8_t *in, uint32_t bitoffset, int32_t numbits )
{
	...
	if ( (numbits + (bitoffset & 0x7)) > 32)
	{
		...
		load2shift = (int32_t)(8-(numbits + (bitoffset & 0x7)-32));
		load2 >>= load2shift;
		result >>= (32-numbits);
		result |= load2;		
	}
	...
	if ( numbits != (int32_t)((sizeof(result) * 8)) )
		result &= ~((uint32_t)0xffffffffu << numbits);
	
	return result;
}


static inline uint32_t dyn_get(uint8_t *in, uint32_t *bitPos, uint32_t m, uint32_t k)
{
...
static inline uint32_t dyn_get_32bit( uint8_t * in, uint32_t * bitPos, uint32_t m, uint32_t k, uint32_t maxbits )
{
	uint32_t	tempbits = *bitPos;
	uint32_t		v;
	uint32_t		streamlong;
	...
	{
		result = getstreambits(in, tempbits+MAX_PREFIX_32, (int32_t)maxbits);
...
int32_t dyn_decomp( AGParamRecPtr params, BitBuffer * bitstream, int32_t * pc, uint32_t numSamples, uint32_t maxSize, uint32_t * outNumBits )
{
    uint8_t 		*in;
    int32_t			*outPtr = pc;
	...
	*outNumBits = 0;
    ...
    while (c < numSamples)
    {
        ...
        // least significant bit is sign bit
        {
            ...
            multiplier |= 1;
            del = (int32_t)((uint32_t)(ndecode+(uint32_t)1u) >> 1) * (multiplier);
        }
        ...
        *outPtr++ = del;
        ...
        if (((mb << MMULSHIFT) < QB) && (c < numSamples))
        {
			...
			#if 0	/* numSamples has restriction by ALACDecoder.c L.156,L.337 - Therefore, it becomes an error by ag_dec.c L.383 */
            if(n >= (uint32_t)65535u)
            	zmode = 0;
			#endif
            mb = 0;
        }
